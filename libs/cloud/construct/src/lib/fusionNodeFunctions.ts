...
export class FusionNodeFunction extends NodejsFunction {
	constructor(scope: Construct, id: string, props: FusionNodejsFunctionProps) {
		...
		const serviceLayerArn = StringParameter.valueForStringParameter(scope, 'service-framework-layer-nodejs-NodeMicroservicesDependenciesLayerArn')
		const serviceLayer = LayerVersion.fromLayerVersionArn(scope, 'FusionNodeFunctionLayer' + id, serviceLayerArn)
		...
		const desiredBundlingSet = new Set(isNeedNodejsLayer ? [...(desiredBundling?.externalModules ?? []), ...layerModules] : [...(desiredBundling?.externalModules ?? [])])
